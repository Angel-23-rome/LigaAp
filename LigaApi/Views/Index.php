    ...
    include '../Models/Conexion.php';
		for ($i = 0; $i < sizeof($rounds); $i++) {
			...
			foreach ($rounds[$i] as $r) {
                $equiposIn= explode(",", $r);
                $sql = "INSERT INTO partidos (Equipo_1,Equipo_2,Jornada_P) VALUES (?,?,?)";
                $stmt= $pdo->prepare($sql);
                $stmt->execute([$equiposIn[0],$equiposIn[1],$jor]);
	...
	echo "<tr>";
		...
		for ($i = sizeof($rounds) - 1; $i >= 0; $i--) {
			...
			foreach ($rounds[$i] as $r) {
                ...
                $sql = "INSERT INTO partidos (Equipo_1,Equipo_2,Jornada_P) VALUES (?,?,?)";
                $stmt= $pdo->prepare($sql);
                $stmt->execute([$equiposIns[0],$equiposIns[1],$round_counter]);
