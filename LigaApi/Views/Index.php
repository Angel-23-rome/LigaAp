    ...
    include '../Models/Conexion.php';
		for ($i = 0; $i < sizeof($rounds); $i++) {
			...
			foreach ($rounds[$i] as $r) {
                $V1=1;
                $equiposIn= explode(",", $r);
                $sql = "INSERT INTO partidos (Equipo_1,Equipo_2,Jornada_P,Torneo_P,Vuelta_P) SELECT $equiposIn[0],$equiposIn[1],$jor,MAX(idTorneo),$V1 FROM torneos";
	...
	echo "<tr>";
		...
		for ($i = sizeof($rounds) - 1; $i >= 0; $i--) {
			...
			foreach ($rounds[$i] as $r) {
                ...
                $sql = "INSERT INTO partidos (Equipo_1,Equipo_2,Jornada_P,Torneo_P,Vuelta_P) SELECT $equiposIns[0],$equiposIns[1],$round_counter,MAX(idTorneo),$V2 FROM torneos";
                $stmt= $pdo->prepare($sql);
                $stmt->execute();
